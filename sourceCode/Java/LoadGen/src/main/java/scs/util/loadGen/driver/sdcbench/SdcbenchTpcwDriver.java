 ...
 *
 */
public class SdcbenchTpcwDriver extends AbstractJobDriver{
	/**
	 * Singleton code block
	 */
	private static SdcbenchTpcwDriver driver=null;	


	public SdcbenchTpcwDriver(){initVariables();}
	public synchronized static SdcbenchTpcwDriver getInstance() {
		if (driver == null) {
			driver = new SdcbenchTpcwDriver();
		}
		return driver;
	}
	...
	protected void initVariables() {
		...
		queryItemsStr="http://Ip:Port/servlet/TPCW_product_detail_servlet?I_ID="; //注意加随机数 line58
		queryItemsStr=queryItemsStr.replace("Ip", "192.168.1.106");
		queryItemsStr=queryItemsStr.replace("Port", "7090");
	}
		...
		while(Repository.onlineDataFlag[serviceId]==true){
			if(Repository.sendFlag[serviceId]==true&&Repository.realRequestIntensity[serviceId]>0){
				...
				for (int i=0;i<Repository.realRequestIntensity[serviceId];i++){
					...
					executor.execute(new LoadExecThread(httpClient,queryItemsStr+rand.nextInt(9999),begin,serviceId,"",sleepUnit*i,"GET"));
				}
