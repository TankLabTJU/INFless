 ...
 *
 */
public class SocialNetworkDriver extends AbstractJobDriver{
	/**
	 * Singleton code block
	 */
	private static SocialNetworkDriver driver=null;	
	
	public SocialNetworkDriver(){initVariables();}
	public synchronized static SocialNetworkDriver getInstance() {
		if (driver == null) {
			driver = new SocialNetworkDriver();
		}
		return driver;
	}
...
//		queryItemsStr=queryItemsStr.replace("Ip", "192.168.1.110");
//		queryItemsStr=queryItemsStr.replace("Port", "30300");
//		queryItemsStr=queryItemsStr.replace("Ip", "10.30.12.2");
//		queryItemsStr=queryItemsStr.replace("Port", "5000");
		...
		while(Repository.onlineDataFlag[serviceId]==true){
			if(Repository.sendFlag[serviceId]==true&&Repository.realRequestIntensity[serviceId]>0){
				...
				for (int i=0;i<Repository.realRequestIntensity[serviceId];i++){ 
					//executor.execute(new LoadExecThreadSocailNetworkSpec(httpClient,queryItemsStr,begin,serviceId,jsonParmStr.replace("x", Integer.toString(random.nextInt(1000))),sleepUnit*i,"POST"));
