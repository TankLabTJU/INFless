 ...
 *
 */
public class HalfTFServingDriver extends AbstractJobDriver{
	/**
	 * Singleton code block
	 */
	private static HalfTFServingDriver driver=null;	
	
	//private StringBuilder builder=new StringBuilder();
	
	public HalfTFServingDriver(){initVariables();}
	public synchronized static HalfTFServingDriver getInstance() {
		if (driver == null) {
			driver = new HalfTFServingDriver();
		}
		return driver;
	}
	...
	protected void initVariables() {
		...
		queryItemsStr=Repository.halfBaseURL;
		jsonParmStr=Repository.halfParmStr;
		queryItemsStr=queryItemsStr.replace("Ip", "192.168.1.105");
		queryItemsStr=queryItemsStr.replace("Port", "30103");
	}
