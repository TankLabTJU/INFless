...
func (c *Client) deploy(context context.Context, spec *DeployFunctionSpec, update bool) (int, string) {
	...
	req := types.FunctionDeployment{
		...
		Image:                  spec.Image,
                QpsPerInstance:         spec.QpsPerInstance,
	...
	if spec.FunctionResourceRequest.Limits != nil && len(spec.FunctionResourceRequest.Limits.CPU) > 0 {
        ...
        if spec.FunctionResourceRequest.Limits != nil && len(spec.FunctionResourceRequest.Limits.GPU_Memory) > 0 {
		hasLimits = true
		req.Limits.GPU_Memory = spec.FunctionResourceRequest.Limits.GPU_Memory
	}
	if spec.FunctionResourceRequest.Limits != nil && len(spec.FunctionResourceRequest.Limits.GPU) > 0 {
		hasLimits = true
		req.Limits.GPU = spec.FunctionResourceRequest.Limits.GPU
	}
	if !hasLimits {
		req.Limits = nil
	}
	...
	if spec.FunctionResourceRequest.Requests != nil && len(spec.FunctionResourceRequest.Requests.CPU) > 0 {
        ...
        if spec.FunctionResourceRequest.Requests != nil && len(spec.FunctionResourceRequest.Requests.GPU_Memory) > 0 {
		hasRequests = true
		req.Requests.GPU_Memory = spec.FunctionResourceRequest.Requests.GPU_Memory
	}
	if spec.FunctionResourceRequest.Requests != nil && len(spec.FunctionResourceRequest.Requests.GPU) > 0 {
		hasRequests = true
		req.Requests.GPU = spec.FunctionResourceRequest.Requests.GPU
	}
	if !hasRequests {
		req.Requests = nil
	}
