...
predic=data(result(1:30),5)-1.8;
autual=data(result(1:30),6);
err=(autual-predic)./autual;
pred=zeros(size(real));
for i=1:length(real);
    pred(i,1)=real(i,1)*(1-err(i,1));
end  
...
% plot(data(result(1:30),5)-1.8,'-', 'LineWidth',lw ,'color',[0 114 189]/255,'Marker','o','MarkerSize',5,'Color',[255 127 14]/255); %红色  
% hold on;
% autual=data(result(1:30),6);
% plot(data(result(1:30),6),'-', 'LineWidth',lw ,'color',[255 0 0]/255,'Marker','o','MarkerSize',4,'Color',[31 119 180]/255); %红色  
...
set(gca,'YLim',[10  30]);%X轴的数据显示范围
set(gca,'YTick',[10 : 10: 40]);%设置要显示坐标刻度
